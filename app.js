...
function updatePriceDisplays() {
  ...
  if (navEl && liveEthPrice)
    navEl.textContent = "$" + liveEthPrice.toLocaleString();
...
async function fetchRecentSales() {
  try {
    ...
    const sales = ev
      ...
      .map((e) => {
        const eth = e.payment
          ? (parseInt(e.payment.quantity) / 1e18).toFixed(4)
          : "0.064";
...
function renderStatsBar() {
  ...
  setEl(
    "stat-volume",
    c.total_volume.toLocaleString(undefined, { maximumFractionDigits: 1 }),
  );
  ...
  if (holdersEl && c.num_owners)
    holdersEl.dataset.countup = String(c.num_owners);
  if (itemsEl && c.total_supply)
    itemsEl.dataset.countup = String(c.total_supply);

  if (c.num_owners && c.total_supply)
    setEl(
      "stat-holders-sub",
      ((c.num_owners / c.total_supply) * 100).toFixed(1) + "% of supply",
    );
  if (c.listed_count && c.total_supply)
    setEl(
      "stat-listed-sub",
      ((c.listed_count / c.total_supply) * 100).toFixed(1) + "% listed",
    );
...
async function fetchTopSales() {
  try {
    const r = await fetch(
      "https://api.opensea.io/api/v2/events/collection/cryptofish?event_type=sale&limit=100&occurred_before=" +
...
function renderFOTD() {
  ...
  setEl(
    ...
    fish.sci
      ...
      : fish.honorary
        ? `Custom honorary token for ${fish.honorary}.`
        : "",
  );
  setHTML(
    "fotd-status",
    `<span class="status-pill status-${fish.status.toLowerCase()}">⬤ ${STATUS_NAMES[fish.status] || "Unknown"}</span>`,
  );
  ...
  if (bb)
    bb.onclick = () =>
      window.open(
        `https://opensea.io/assets/ethereum/0x9ef31ce8cca614e7aff3c1b883740e8d2728fe91/${fish.tokenId}`,
        "_blank",
      );
}
...
function showFish(idx) {
  ...
  setEl(
    "sp-status",
    (STATUS_NAMES[f.status] || "Unknown") + " (" + f.status + ")",
  );
  ...
  if (buyBtn)
    buyBtn.onclick = () =>
      window.open(
        `https://opensea.io/assets/ethereum/0x9ef31ce8cca614e7aff3c1b883740e8d2728fe91/${f.tokenId}`,
        "_blank",
      );
  ...
  const sameGenus = f.genus
    ? FISH_DATA.filter((x) => x.genus === f.genus).length
    : 0;
  const sameLocality = f.locality
    ? FISH_DATA.filter((x) => x.locality === f.locality).length
    : 0;
  ...
  if (ctx) {
    ...
    if (f.genus)
      parts.push(
        `<strong>${sameGenus}</strong> in genus <em>${escapeHTML(f.genus)}</em>`,
      );
    if (f.locality)
      parts.push(
        `<strong>${sameLocality}</strong> from ${escapeHTML(f.locality)}`,
      );
    parts.push(
      `<strong>${sameStatus}</strong> ${STATUS_NAMES[f.status] || "Unknown"}`,
    );
  ...
  if (histEl)
    histEl.textContent = f.honorary
      ? `Custom honorary token created for ${f.honorary}.`
      : f.sci
        ? `${f.genus} ${f.species}, found in ${f.locality || "its native habitat"}.`
        : "";
  if (consEl)
    consEl.textContent =
...
function getFilteredFish() {
  ...
  if (activeFilters.locality.length)
    fish = fish.filter((f) =>
      activeFilters.locality.some((l) => f.locality === l),
    );
  ...
  switch (currentSort) {
    ...
    case "status":
      fish = [...fish].sort(
        (a, b) => (so[a.status] ?? 9) - (so[b.status] ?? 9),
      );
      break;
    default:
      fish = [...fish].sort((a, b) => a.tokenId - b.tokenId);
  }
  return fish;
}
...
function renderLibrary() {
  ...
  if (countEl)
    countEl.textContent = `Showing ${fish.length} of ${allFish.length} fish`;
}
...
function filterPills(containerId, query) {
  ...
  document
    .querySelectorAll(`#${containerId} .filter-pill`)
    .forEach((p) =>
      p.classList.toggle(
        "hidden",
